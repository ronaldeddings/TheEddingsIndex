...
public struct FathomClient: Sendable {
    ...
    public func indexSingleFile(path: String) throws -> [Int64] {
        ...
        if let meetingDate = frontmatter.date, meetingDate < DataPolicy.cutoffDate { return [] }
        ...
        logger.info("FathomClient indexed \(insertedIds.count) chunks from \(URL(filePath: path).lastPathComponent)")
        return insertedIds
    }
}
